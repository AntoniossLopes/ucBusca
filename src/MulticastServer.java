...
public class MulticastServer extends Thread {
            ...
            if (aux0.length == 1) {
                switch (aux0[0]) {
                    case "type|info":
                        ...
                        if (provisorio_sites.size() == 0) {
                            buildInfo.append("popular_list|url_list;item_count|1;url|No urls Found");
                            buildInfo.append("^");
                        }
                        ...
                        if (popular_search.size() != 0) {
                            ...
                            for (Word word : popular_search) {
                                if (i == 10) {
                                    ...
                                    buildInfo.append("word_" + i + "|").append(word.getWord()).append(";");
                                    i = i + 1;
                                }
                            }
                            buildInfo.append("^");
                        ...
                        if (popular_search.size() == 0) {
                            buildInfo.append("popular_list|word_list;item_count|1;word|No Words Found;");
                            buildInfo.append("^");
                        }
                ...
                switch (aux0[0]) {
                    ...
                    case "type|serverid":
                        ...
                        break;
                    case "type|associate":
                        String[] usernome = aux0[1].split("\\|");
                        String[] idfb = aux0[2].split("\\|");
                        String[] token = aux0[3].split("\\|");
                        for (User user : users) {
                            if (user.getNome().equals(usernome[1])) {
                                user.setFbID(idfb[1]);
                                user.setFbToken(token[1]);
                            }
                        }
                        break;

                    case "type|searchidfb":
                        String[] fb = aux0[1].split("\\|");
                        ...
                        if (verify.equals("Done")) {
                            ...
                            for (User useraux : users) {
                                if (useraux.getNome().equals(nome2[1])) {
                                    ...
                                    fbId = useraux.getFbID();
                                    fbToken = useraux.getFbToken();
                                    useraux.login();
                                }
                            }
    ...
    public synchronized void SearchRecursive(int maxSearch) throws InterruptedException {
